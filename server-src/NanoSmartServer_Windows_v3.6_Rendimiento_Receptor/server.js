...
function loadConfig() {
  ...
  parsed.production.authFailureLimit = Number.parseInt(
    parsed.production.authFailureLimit || 10,
    10
  );
  ...
  parsed.production.authBlockSeconds = Number.parseInt(
    parsed.production.authBlockSeconds || 900,
    10
  );
  ...
  parsed.protocol.duplicateWindowSeconds = Number.parseInt(
    parsed.protocol.duplicateWindowSeconds || 300,
    10
  );
  ...
  parsed.commands.endpointMaxAgeSeconds = Number.parseInt(
    parsed.commands.endpointMaxAgeSeconds || 90,
    10
  );
  parsed.commands.inquiryRetrySeconds = Number.parseInt(
    parsed.commands.inquiryRetrySeconds || 45,
    10
  );
  parsed.commands.commandTtlSeconds = Number.parseInt(
    parsed.commands.commandTtlSeconds || 180,
    10
  );
...
function pruneProductionAuthAttempts(now = Date.now()) {
  const staleAfter = Math.max(
    config.production.authFailureWindowSeconds,
    config.production.authBlockSeconds
  ) * 2000;
  for (const [ip, state] of productionAuthAttempts) {
    if (state.blockedUntil <= now && now - state.windowStartedAt > staleAfter) {
      productionAuthAttempts.delete(ip);
    }
  }
}
...
function authenticateProduction(req) {
  if (!productionCredential) {
    throw new RegistryError('El acceso de producción está desactivado', 503, 'PRODUCTION_DISABLED');
  }
  ...
  if (state.blockedUntil > now) {
    ...
    throw new RegistryError(
      'Acceso temporalmente bloqueado por intentos incorrectos',
      429,
  ...
  if (!supplied || !secretsEqual(supplied, productionCredential.value)) {
    ...
    if (state.failures >= config.production.authFailureLimit) {
      state.blockedUntil = now + config.production.authBlockSeconds * 1000;
    }
    appendProductionAudit({
      at: new Date(now).toISOString(),
      action: 'PRODUCTION_AUTH_DENIED',
      sourceIp,
...
function importSupportDatabase(databaseBuffer, sourceIp) {
  ...
  if (!Buffer.isBuffer(databaseBuffer) || databaseBuffer.length < sqliteHeader.length ||
      !databaseBuffer.subarray(0, sqliteHeader.length).equals(sqliteHeader)) {
    throw new RegistryError('El archivo seleccionado no es una base support.db valida', 400, 'INVALID_SUPPORT_DATABASE');
  }

  const temporaryPath = path.join(
    config.storage.directory,
    `.support-import-${process.pid}-${crypto.randomUUID()}.db`
  );
  try {
    ...
    appendProductionAudit({
      ...
      action: 'ADMIN_IMPORT_SUPPORT_DB',
    ...
    throw new RegistryError(
      'No se pudo leer la tabla clients de support.db',
      400,
      'INVALID_SUPPORT_DATABASE'
    );
  } finally {
...
function isDuplicate(raw, now) {
  ...
  if (now - lastRecentHashPruneAt >= pruneEveryMs) {
    ...
    for (const [key, timestamp] of recentHashes) {
      if (now - timestamp > windowMs) recentHashes.delete(key);
    }
  }

  return {
    hash,
    duplicate: Number.isFinite(previousAt) && now - previousAt <= windowMs
  };
}
...
async function forwardAlertToPhones(event) {
  ...
  if (result.sent > 0) {
    console.log(`[FCM] Alerta ${event.id} enviada a ${result.sent}/${result.requested} celular(es) del IMEI ${event.imei}`);
  } else if (result.requested === 0) {
    console.log(`[FCM] IMEI ${event.imei} sin celulares con token push registrado`);
  } else if (result.skipped > 0) {
    console.warn(`[FCM] Envío omitido: ${pushService.getStatus().error || 'servicio no disponible'}`);
  } else {
    console.warn(`[FCM] No se pudo enviar la alerta ${event.id}: ${(result.errorMessages || []).join(' | ')}`);
  }
}
...
async function forwardCommandToPhones(command) {
  ...
  if (result.sent > 0) {
    console.log(`[FCM] Estado ${command.action} enviado a ${result.sent}/${result.requested} celular(es)`);
  } else if (result.requested > 0 && result.errors > 0) {
    console.warn(`[FCM] No se pudo notificar la orden ${command.id}: ${(result.errorMessages || []).join(' | ')}`);
  }
}
...
async function forwardPanelStateToPhones(panelState) {
  ...
  if (result.sent > 0) {
    console.log(
      `[FCM] Cambio del panel ${panelState.panelStatus} enviado a ` +
      `${result.sent}/${result.requested} celular(es)`
    );
  } else if (result.requested > 0 && result.errors > 0) {
    console.warn(
      `[FCM] No se pudo notificar el estado del panel ${panelState.imei}: ` +
...
async function trySendInquiry(imei, options = {}) {
  ...
  try {
    ...
    console.log(
      `[CONTROL] DI01 enviado al IMEI ${imei} en ${normalizedIp(endpoint.address)}:${endpoint.port} ` +
      `(intento ${command.attempts + 1})`
    );
...
function sendAck(packet, remote, now, action = null) {
  if (!config.protocol.ack.enabled) {
    return Promise.resolve({ sent: false, payload: null, error: null, disabled: true });
  }

  const payload = buildAck(packet, {
    startMarker: config.protocol.startMarker,
    endMarker: config.protocol.endMarker,
    timeZone: config.protocol.ack.timeZone,
    lineEnding: config.protocol.ack.lineEnding,
    now: new Date(now),
    action
  });
  ...
  if (!payload) {
    return Promise.resolve({
      sent: false,
      payload: null,
      error: 'No se pudo construir el ACK: faltan identificador o secuencia',
      disabled: false
    });
  }
  ...
  return new Promise((resolve) => {
    udpServer.send(Buffer.from(payload, 'utf8'), remote.port, remote.address, (error) => {
      if (error) {
        runtimeState.ackErrors += 1;
        resolve({ sent: false, payload, error: error.message, disabled: false });
        return;
      }
...
async function handleDatagram(message, remote) {
  ...
  if (message.length > config.udp.maxPacketBytes) {
    console.warn(`[UDP] Paquete descartado por tamaño: ${message.length} bytes`);
    return;
  }
  ...
  if (heartbeatPacket) {
    const endpoint = commandService.recordEndpoint(heartbeatPacket.imei, remote, {
      now,
      source: 'heartbeat'
    });
    console.log(
      `[HEARTBEAT] IMEI ${heartbeatPacket.imei} | ` +
      `${normalizedIp(endpoint.address)}:${endpoint.port} | sin ACK`
    );
    sendSse('device-heartbeat', {
      imei: heartbeatPacket.imei,
      receivedAt: endpoint.lastSeenAt,
      address: normalizedIp(endpoint.address),
      port: endpoint.port,
      heartbeatCount: endpoint.heartbeatCount,
      endpointChanged: endpoint.changed,
      raw: heartbeatPacket.raw
    });
  ...
  if (parsed.imei) {
    commandService.recordEndpoint(parsed.imei, remote, { now, source: 'packet' });
  }
  ...
  if (commandForAck) {
    if (ack.sent) {
      commandUpdate = commandService.markActionSent(commandForAck.id, {
        now,
        transmitterId: parsed.transmitterId,
        sequence: parsed.sequence
      });
      console.log(
        `[CONTROL] ${commandUpdate.action} enviado al IMEI ${commandUpdate.imei} ` +
        `con ACK=${parsed.sequence}; esperando confirmación 43/44/45/46`
      );
      sendSse('device-command', commandUpdate);
    } else {
      commandService.markActionSendFailed(commandForAck.id, ack.error);
      console.warn(
        `[CONTROL] No se pudo entregar ${commandForAck.action} al IMEI ${commandForAck.imei}: ` +
        `${ack.error || 'ACK no enviado'}`
      );
    }
  }

  if (!duplicateCheck.duplicate && parsed.imei && parsed.gprsPanelResult) {
    const confirmation = commandService.recordPanelResult(parsed.imei, parsed.gprsPanelResult, {
      now,
    ...
    if (confirmation.matched) {
      ...
      console.log(
        `[CONTROL] ${commandUpdate.resultDescription} | IMEI ${commandUpdate.imei} | ` +
        `evento ${commandUpdate.resultCode}`
      );
      sendSse('device-command', commandUpdate);
      void forwardCommandToPhones(commandUpdate).catch((error) => {
        runtimeState.pushErrors += 1;
        console.error(`[FCM] Error notificando estado del equipo: ${error.stack || error.message}`);
      });
    } else {
      console.log(
        `[ESTADO PANEL] ${confirmation.panelState.resultDescription} | IMEI ${parsed.imei} | ` +
        `evento ${confirmation.panelState.resultCode}`
      );
      void forwardPanelStateToPhones(confirmation.panelState).catch((error) => {
        runtimeState.pushErrors += 1;
        console.error(`[FCM] Error notificando estado del panel: ${error.stack || error.message}`);
      });
    }
  }
  ...
  if (duplicateCheck.duplicate) {
    ...
    console.log(
      `[UDP DUPLICADO] ${normalizedIp(remote.address)}:${remote.port} | ` +
      `${parsed.imei || 'sin IMEI'} | ${ackText} | no almacenado`
    );
    sendSse('duplicate', {
  ...
  const event = store.append({
    receivedAt: new Date(now).toISOString(),
    sourceIp: normalizedIp(remote.address),
    sourcePort: remote.port,
    rawBytes: message.length,
    packetHash: duplicateCheck.hash,
    isDuplicate: false,
    valid: parsed.valid,
    ...
    sequence: parsed.sequence,
    deviceTimestampRaw: parsed.deviceTimestampRaw,
    packetType: parsed.packetType,
    packetTypeDescription: parsed.packetTypeDescription,
    eventCode: parsed.eventCode,
    eventDescription: parsed.eventDescription,
    contactId: contact.raw,
    contactIdValid: contact.valid,
    contactIdErrors: contact.errors,
    contactQualifier: contact.qualifier,
    contactQualifierDescription: contact.qualifierDescription,
    contactCategory: contact.category,
    partition: contact.partition,
    partitionNumber: contact.partitionNumber,
    subject: contact.subject,
    subjectNumber: contact.subjectNumber,
    subjectKind: contact.subjectKind,
    zoneName: contact.subjectKind === 'ZONA'
      ? registry.getZoneName(parsed.imei, contact.subjectNumber)
      : null,
    checksum: contact.checksum,
    checksumValidated: contact.checksumValidated,
    abonado: parsed.abonado,
    imei: parsed.imei,
    firmware: parsed.firmware,
    channel: parsed.channel,
    shouldForwardToApp: parsed.shouldForwardToApp,
    ackSent: ack.sent,
  ...
  const deliverToApp = event.shouldForwardToApp &&
    registry.shouldDeliverEventToApp(event.imei, event.eventCode);
  const forwarding = deliverToApp ? ' | ALERTA APP' : '';
  console.log(
    `[UDP ${validity}] ${event.sourceIp}:${event.sourcePort} | IMEI ${event.imei || 'desconocido'} | ` +
    `${account} | ${event.eventDescription} | ${ackText}${forwarding} | ${event.raw}`
  );
  sendSse('packet', event);
  if (deliverToApp) {
    sendSse('app-alert', event);
    void forwardAlertToPhones(event).catch((error) => {
      runtimeState.pushErrors += 1;
      console.error(`[FCM] Error inesperado: ${error.stack || error.message}`);
    });
  }
  if (parsed.imei && !commandForAck) {
    await trySendInquiry(parsed.imei, { now });
  }
}
...
function compactAppAlert(event) {
  const zoneName = String(event.subjectKind || '').toUpperCase() === 'ZONA'
    ? registry.getZoneName(event.imei, event.subjectNumber ?? event.subject)
    : null;
  return {
    id: event.id,
    ...
    locationAccuracyMeters: Number.isFinite(event.locationAccuracyMeters)
      ? event.locationAccuracyMeters
      : null,
    locationCapturedAt: event.locationCapturedAt || null,
...
function appDeviceStatus(installation) {
  ...
  return {
    installation,
    device: store.getDevice(installation.imei),
    gprs: {
      online: endpoint?.fresh === true,
      lastSeenAt: endpoint?.lastSeenAt || null
    },
    panelState: commandService.getPanelState(installation.imei),
    latestCommand: commandService.getLatestCommand(installation.imei)
  };
}
...
function authenticateBatch(accessTokens) {
  if (!Array.isArray(accessTokens) || accessTokens.length === 0) {
    throw new RegistryError(
      'Debe enviar al menos un token de panel',
      400,
      'EMPTY_BATCH'
    );
  }
  if (accessTokens.length > config.http.maxBatchItems) {
    throw new RegistryError(
      `La consulta agrupada admite hasta ${config.http.maxBatchItems} paneles`,
      400,
  ...
  return accessTokens.map((value) => {
    ...
    if (!authenticated.has(accessToken)) {
      authenticated.set(accessToken, registry.authenticate(accessToken));
    }
...
function emergencyLocation(body, receivedAt) {
  ...
  if (mapsUrl(latitude, longitude) === null) {
    return {
      latitude: null,
      longitude: null,
  ...
  return {
    latitude,
    longitude,
    locationAccuracyMeters: accuracy !== null && accuracy >= 0 && accuracy <= 100000
      ? accuracy
      : null,
    locationCapturedAt: Number.isFinite(parsedCapturedAt)
      ? new Date(parsedCapturedAt).toISOString()
      : receivedAt
  };
}
...
function appendAppEmergency(installation, body, req) {
  ...
  const abonado = String(
    body.abonado || store.getDevice(installation.imei)?.abonado || ''
  ).trim().slice(0, 32) || null;
  return store.append({
    receivedAt,
    ...
    valid: true,
    parseErrors: [],
    raw: `APP:${emergencyType}`,
    transmitterId: 'APP',
    protocol: 'APP_HTTP',
    sequence: null,
    deviceTimestampRaw: null,
    packetType: 'APP',
    packetTypeDescription: 'Botón de emergencia desde la aplicación',
    eventCode: definition.eventCode,
    eventDescription: definition.description,
    contactId: null,
    contactIdValid: false,
    contactIdErrors: [],
    contactQualifier: '1',
    contactQualifierDescription: 'Evento iniciado desde la aplicación',
    contactCategory: 'ALARMA',
    partition: '01',
    partitionNumber: 1,
    subject: '000',
    subjectNumber: 0,
    ...
    ackPayload: null,
    ackError: null,
    fields: [emergencyType, installation.installationId],
    actionSource: 'APP',
    actorName: installation.name,
...
function getHealth() {
  ...
  return {
    ...
    udp: {
      ...
      listening: runtimeState.udpListening,
      error: runtimeState.udpError
    },
    ...
    production: {
      ...
      listening: runtimeState.productionListening,
      authFailures: runtimeState.productionAuthFailures,
      blockedRequests: runtimeState.productionBlockedRequests
    },
    ack: {
      enabled: config.protocol.ack.enabled,
      timeZone: config.protocol.ack.timeZone,
      sent: runtimeState.acksSent,
      errors: runtimeState.ackErrors
    },
    appForwarding: config.protocol.appForwarding,
    gprsControl: {
      enabled: config.commands.enabled,
      endpointMaxAgeSeconds: config.commands.endpointMaxAgeSeconds,
      inquiryRetrySeconds: config.commands.inquiryRetrySeconds,
      commandTtlSeconds: config.commands.commandTtlSeconds,
      ...commandService.getStats()
    },
    registry: {
      provisionedDevices: provisionedDevices.length,
      activeInstallations: provisionedDevices.reduce((total, device) => total + device.installations, 0),
      pushTokens: registry.countPushTokens()
    },
    firebasePush: {
      ...pushService.getStatus(),
      alertsProcessed: runtimeState.pushAlertsProcessed,
      commandUpdatesProcessed: runtimeState.commandPushesProcessed,
      panelStateUpdatesProcessed: runtimeState.panelStatePushesProcessed,
      messagesSent: runtimeState.pushMessagesSent,
      errors: runtimeState.pushErrors,
      invalidTokensRemoved: runtimeState.invalidPushTokensRemoved
    },
...
function readJsonBody(req, maxBytes = 65536) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (chunk) => {
      total += chunk.length;
      if (total > maxBytes) {
        reject(new RegistryError('Cuerpo de solicitud demasiado grande', 413, 'BODY_TOO_LARGE'));
        req.destroy();
        return;
      }
      chunks.push(chunk);
    ...
    req.on('end', () => {
      ...
      try {
        ...
        reject(new RegistryError('El cuerpo debe ser JSON válido', 400, 'INVALID_JSON'));
      }
    });
    req.on('error', reject);
  });
...
function readBinaryBody(req, maxBytes = 10 * 1024 * 1024) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (chunk) => {
      total += chunk.length;
      if (total > maxBytes) {
        reject(new RegistryError('El archivo support.db es demasiado grande', 413, 'BODY_TOO_LARGE'));
        req.destroy();
        return;
      }
      chunks.push(chunk);
...
function expectedErrorResponse(res, error) {
  if (error instanceof RegistryError || error instanceof CommandError) {
...
const httpServer = http.createServer(async (req, res) => {
  try {
    ...
    if (config.http.restrictAdministrationToPrivateNetworks &&
        !privateClient && !isPublicAppPath(pathname)) {
      json(res, 403, {
        error: 'Esta ruta sólo está disponible desde el servidor o la red privada'
      });
      return;
    }
    ...
    if (req.method === 'GET' && (pathname === '/admin' || pathname === '/admin.html')) {
      res.writeHead(200, {
        'Content-Type': 'text/html; charset=utf-8',
        'Content-Length': adminHtml.length,
        'Cache-Control': 'no-store'
      });
      res.end(adminHtml);
      return;
    ...
    if (req.method === 'GET' && (pathname === '/app-simulator' || pathname === '/app-simulator.html')) {
      res.writeHead(200, {
        'Content-Type': 'text/html; charset=utf-8',
        'Content-Length': appSimulatorHtml.length,
        'Cache-Control': 'no-store'
      });
      res.end(appSimulatorHtml);
      return;
    }

    const provisionMatch = pathname.match(/^\/api\/admin\/devices\/(\d{15})\/access-key$/);
    if (req.method === 'POST' && provisionMatch) {
      json(res, 201, await provisionPairingLabel(provisionMatch[1]));
      return;
    }

    if (req.method === 'POST' && pathname === '/api/admin/installations') {
      const body = await readJsonBody(req);
      json(res, 201, registry.registerInstallationFromAdmin(body));
      return;
    }

    if (req.method === 'GET' && pathname === '/api/admin/devices') {
      json(res, 200, { devices: registry.listProvisionedDevices() });
      return;
    }

    if (req.method === 'GET' && pathname === '/api/admin/clients') {
      json(res, 200, { clients: registry.listClients() });
      return;
    }

    if (req.method === 'POST' && pathname === '/api/admin/clients/import-support-db') {
      const databaseBuffer = await readBinaryBody(req);
      const imported = importSupportDatabase(databaseBuffer, normalizedIp(req.socket.remoteAddress));
      console.log(`[ADMIN] Base support.db actualizada: ${imported.total} clientes`);
    ...
    if (req.method === 'POST' && pathname === '/api/admin/clients/robbery-forwarding') {
      ...
      const client = registry.setClientRobberyForwarding(body.clientId, body.enabled === true);
      appendProductionAudit({
        at: new Date().toISOString(),
        action: 'ADMIN_SET_CLIENT_ROBBERY_FORWARDING',
        clientId: client.id,
        enabled: client.forwardRobberyToApp,
        sourceIp: normalizedIp(req.socket.remoteAddress)
      });
      json(res, 200, { client });
      return;
    ...
    if (req.method === 'GET' && installationsMatch) {
      json(res, 200, {
        imei: installationsMatch[1],
        installations: registry.listInstallations(installationsMatch[1])
      });
      return;
    }
    ...
    if (req.method === 'POST' && revokeMatch) {
      json(res, 200, registry.revokeInstallation(revokeMatch[1]));
      return;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/app/batch/status') {
      ...
      const installations = authenticateBatch(body.accessTokens);
      json(res, 200, {
        count: installations.length,
        statuses: installations.map(appDeviceStatus)
      });
      return;
    }
    ...
    if (req.method === 'POST' && pathname === '/api/app/batch/push-token') {
      const body = await readJsonBody(req, 1024 * 1024);
      const installations = authenticateBatch(body.accessTokens);
      const updated = installations.map((installation) =>
        registry.updatePushToken(
          installation.installationId,
          body.pushToken,
          body.platform
        )
      );
      json(res, 200, { count: updated.length, installations: updated });
      return;
    }
    ...
    if (pathname.startsWith('/api/app/')) {
      ...
      if (req.method === 'GET' && pathname === '/api/app/me') {
        json(res, 200, installation);
        return;
      }

      if (req.method === 'POST' && pathname === '/api/app/me/name') {
        const body = await readJsonBody(req);
        json(res, 200, registry.updateInstallationName(installation.installationId, body.name));
        return;
      }

      if (req.method === 'GET' && pathname === '/api/app/device/status') {
        json(res, 200, appDeviceStatus(installation));
        return;
      }

      if (req.method === 'GET' && pathname === '/api/app/device/zones') {
        json(res, 200, {
          imei: installation.imei,
          zones: registry.getZoneNames(installation.imei)
        });
        return;
      }

      if (req.method === 'PUT' && pathname === '/api/app/device/zones') {
        const body = await readJsonBody(req);
        json(res, 200, {
          imei: installation.imei,
          zones: registry.updateZoneNames(installation.imei, body.zones)
        });
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/app/device/command') {
        if (!config.commands.enabled) {
          throw new CommandError('El control GPRS está desactivado', 503, 'COMMANDS_DISABLED');
        }
        const body = await readJsonBody(req);
        const currentInstallation = body.name
          ? registry.updateInstallationName(installation.installationId, body.name)
          : installation;
        const created = commandService.createCommand(
          currentInstallation.imei,
          body.action,
          currentInstallation.installationId,
          currentInstallation.name
        );
        await trySendInquiry(currentInstallation.imei);
        const command = commandService.getCommand(created.id, currentInstallation.imei);
      ...
      if (req.method === 'POST' && pathname === '/api/app/emergency') {
        ...
        console.log(
          `[APP] ${event.eventDescription} ejecutado por ${event.actorName} | IMEI ${event.imei}`
        );
        sendSse('packet', event);
        sendSse('app-alert', event);
        void forwardAlertToPhones(event).catch((error) => {
          runtimeState.pushErrors += 1;
          console.error(`[FCM] Error notificando emergencia: ${error.stack || error.message}`);
        });
        json(res, 201, { alert: compactAppAlert(event) });
        return;
      }
      ...
      if (req.method === 'GET' && commandStatusMatch) {
        json(res, 200, {
          command: commandService.getCommand(commandStatusMatch[1], installation.imei)
        });
        return;
      }
      ...
      if (req.method === 'GET' && pathname === '/api/app/alerts') {
        const requestedTake = Math.min(
          Math.max(Number.parseInt(requestUrl.searchParams.get('take') || '100', 10) || 100, 1),
          1000
        );
        const alerts = store.getEvents({
          take: 1000,
          afterId: requestUrl.searchParams.get('afterId'),
          imei: installation.imei,
          forwardOnly: true
        })
          .filter((event) => registry.shouldDeliverEventToApp(event.imei, event.eventCode))
          .slice(0, requestedTake);
        json(res, 200, {
          imei: installation.imei,
          alerts: alerts.map(compactAppAlert)
        });
        return;
      }
      ...
      if (req.method === 'POST' && pathname === '/api/app/push-token') {
        ...
        json(
          res,
          200,
          registry.updatePushToken(installation.installationId, body.pushToken, body.platform)
        );
        return;
      }
    }
    ...
    if (req.method === 'GET' && pathname === '/api/health') {
      ...
      if (config.http.restrictAdministrationToPrivateNetworks && !privateClient) {
        json(res, 200, {
          ok: health.ok,
          service: health.service,
          version: health.version,
          udpListening: health.udp.listening,
          httpListening: health.http.listening,
          firebaseReady: health.firebasePush.ready
        });
      } else {
        json(res, 200, health);
      }
    ...
    if (req.method === 'GET' && pathname === '/api/gprs/endpoints') {
      const endpoints = commandService.listEndpoints().map((endpoint) => ({
        ...endpoint,
        panelState: commandService.getPanelState(endpoint.imei),
        latestCommand: commandService.getLatestCommand(endpoint.imei)
      }));
      json(res, 200, {
        endpoints,
        heartbeatTotal: commandService.getStats().heartbeatsReceived,
        endpointMaxAgeSeconds: config.commands.endpointMaxAgeSeconds
      });
      return;
    }
    ...
    if (req.method === 'GET' && pathname === '/api/events') {
      const events = store.getEvents({
        ...
        afterId: requestUrl.searchParams.get('afterId'),
        abonado: requestUrl.searchParams.get('abonado'),
        imei: requestUrl.searchParams.get('imei'),
        forwardOnly: requestUrl.searchParams.get('forwardOnly')
      });
      json(res, 200, { events });
      return;
    }
    ...
    if (req.method === 'GET' && pathname === '/api/devices') {
      json(res, 200, { devices: store.getDevices() });
      return;
    }

    const deviceStatusMatch = pathname.match(/^\/api\/devices\/(\d{15})\/status$/);
    if (req.method === 'GET' && deviceStatusMatch) {
      const device = store.getDevice(deviceStatusMatch[1]);
      if (!device) {
        json(res, 404, { error: 'IMEI no encontrado' });
        return;
      }
      json(res, 200, device);
      return;
    ...
    if (req.method === 'GET' && deviceEventsMatch) {
      const events = store.getEvents({
        take: requestUrl.searchParams.get('take'),
        afterId: requestUrl.searchParams.get('afterId'),
        imei: deviceEventsMatch[1]
      });
      json(res, 200, { events });
      return;
    }
    ...
    if (req.method === 'GET' && deviceAlertsMatch) {
      const events = store.getEvents({
        take: requestUrl.searchParams.get('take'),
        afterId: requestUrl.searchParams.get('afterId'),
        imei: deviceAlertsMatch[1],
        forwardOnly: true
      });
      json(res, 200, { imei: deviceAlertsMatch[1], alerts: events });
      return;
    }

    if (req.method === 'GET' && pathname === '/api/accounts') {
      json(res, 200, { accounts: store.getAccounts() });
      return;
    }

    const accountStatusMatch = pathname.match(/^\/api\/accounts\/([^/]+)\/status$/);
    if (req.method === 'GET' && accountStatusMatch) {
      const account = store.getAccount(accountStatusMatch[1]);
      if (!account) {
        json(res, 404, { error: 'Abonado no encontrado' });
        return;
      }
    ...
    if (req.method === 'GET' && accountEventsMatch) {
      const events = store.getEvents({
        take: requestUrl.searchParams.get('take'),
        afterId: requestUrl.searchParams.get('afterId'),
        abonado: accountEventsMatch[1]
      });
      json(res, 200, { events });
      return;
    }
...
const productionHttpServer = http.createServer({
  ...
  try {
    ...
    const pathname = decodeURIComponent(requestUrl.pathname);
    const knownRoute =
      (req.method === 'GET' && pathname === '/api/production/health') ||
      (req.method === 'GET' && pathname === '/api/production/clients') ||
      (req.method === 'POST' && pathname === '/api/production/pairings');

    if (!knownRoute) {
      productionJson(res, 404, { error: 'Ruta no encontrada' });
      return;
    }

    authenticateProduction(req);
    if (req.method === 'GET' && pathname === '/api/production/health') {
      productionJson(res, 200, {
        ok: true,
        service: 'NanoSmart Producción',
        version: '1.4.0',
        serverVersion: '3.6.0'
      });
      return;
    ...
    if (req.method === 'GET' && pathname === '/api/production/clients') {
      productionJson(res, 200, { clients: registry.listClients() });
      return;
    }

    const body = await readJsonBody(req, 16384);
    const provisioned = registry.provisionMobilePairings(
      body.imei,
      body.labels,
      { reissue: body.reissue === true, clientId: body.clientId }
    );
    const result = await createProductionPairingLabels(provisioned);
    appendProductionAudit({
      at: new Date().toISOString(),
      action: 'GENERATE_TWO_MOBILE_QR',
      imei: result.imei,
      clientId: provisioned.client.id,
      clientName: provisioned.client.name,
      labels: result.pairings.map((item) => item.label),
      sourceIp: normalizedIp(req.socket.remoteAddress)
    });
    console.log(`[PRODUCCIÓN] IMEI ${result.imei}: dos QR móviles generados`);
    productionJson(res, 201, result);
  } catch (error) {
    if (error instanceof RegistryError || error instanceof CommandError) {
      productionJson(res, error.statusCode, { error: error.message, code: error.code });
      return;
    }
    console.error(`[PRODUCCIÓN HTTP] ${error.stack || error.message}`);
    if (!res.headersSent) productionJson(res, 500, { error: 'Error interno del servidor' });
    else res.end();
  }
});
...
udpServer.on('message', (message, remote) => {
  void handleDatagram(message, remote).catch((error) => {
    console.error(`[UDP] Error procesando paquete: ${error.stack || error.message}`);
  });
});
udpServer.on('error', (error) => {
  runtimeState.udpListening = false;
  runtimeState.udpError = error.code || error.message;
  console.error(`[UDP] ${error.stack || error.message}`);
});
...
productionHttpServer.on('error', (error) => {
  runtimeState.productionListening = false;
  console.error(`[PRODUCCIÓN HTTP] ${error.stack || error.message}`);
  process.exitCode = 1;
});
...
function lanAddresses() {
  ...
  try {
    ...
    console.warn(`[RED] No se pudieron enumerar las IP locales: ${error.message}`);
  }
  return addresses;
}

udpServer.bind(config.udp.port, config.udp.host, () => {
  runtimeState.udpListening = true;
  runtimeState.udpError = null;
  console.log('============================================================');
  ...
  console.log(
    `[CONTROL] Heartbeat IMEI sin ACK - DI01 + ARMAR/DESARMAR ` +
    `(orden válida ${config.commands.commandTtlSeconds}s)`
  );
  const pushStatus = pushService.getStatus();
  if (pushStatus.ready) {
    console.log(`[FCM] Activo - proyecto ${pushStatus.projectId}`);
  } else if (pushStatus.enabled) {
    console.warn(`[FCM] Pendiente de configuración - ${pushStatus.error}`);
  } else {
    console.log('[FCM] Desactivado');
  }
});
...
httpServer.listen(config.http.port, config.http.host, () => {
  ...
  for (const ip of lanAddresses()) {
    console.log(`[WEB] Panel en red: http://${ip}:${config.http.port}`);
  }
...
if (config.production.enabled) {
  productionHttpServer.listen(config.production.port, config.production.host, () => {
    ...
    console.log(
      `[PRODUCCIÓN] API aislada en ${config.production.host}:${config.production.port} ` +
      `(no disponible en TCP ${config.http.port})`
    );
    console.log('[PRODUCCIÓN] Use MOSTRAR_CLAVE_PRODUCCION.bat para activar la estación.');
  });
} else {
  console.log('[PRODUCCIÓN] API desactivada');
}
...
async function shutdown(signal) {
  ...
  const serversClosed = Promise.all(servers.map((server) => new Promise((resolve) => {
  ...
  try {
    ...
    console.error(`[ALMACENAMIENTO] Error al finalizar: ${error.message}`);
  }
  const graceful = await Promise.race([
    serversClosed.then(() => true),
